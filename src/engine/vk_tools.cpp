...
void createBuffer(RazWindow* window,
VkDeviceSize size,
...
VkDeviceMemory& bufferMemory) {
    ...
    if (vkAllocateMemory(window->device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) {
...
VkCommandBuffer beginSingleTimeCommands(struct Engine_App* state) {
    ...
    vkAllocateCommandBuffers(state->window.device, &allocInfo, &commandBuffer);
...
void endSingleTimeCommands(struct Engine_App* state, VkCommandBuffer commandBuffer) {
    ...
    vkFreeCommandBuffers(state->window.device, state->commandPool, 1, &commandBuffer);
}
...
VkFormat findSupportedFormat(struct Engine_App* state, const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
    for (VkFormat format : candidates) {
        ...
        vkGetPhysicalDeviceFormatProperties(state->window.physicalDevice, format, &props);
...
VkFormat findDepthFormat(struct Engine_App* state) {
    return findSupportedFormat(state, { VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT },
...
VkSampleCountFlagBits getMaxUsableSampleCount(struct Engine_App* state){
    ...
    vkGetPhysicalDeviceProperties(state ->window.physicalDevice, &physicalDeviceProperties);
